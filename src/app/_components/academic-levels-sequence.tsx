    ...
    <Reveal className={className} stagger={0.12}>
      <ol className="grid gap-x-8 gap-y-14 sm:grid-cols-2">
        {panels.map((panel, index) => (
          <li
            className={cn("flex flex-col group", index % 2 === 1 && "sm:mt-20")}
            data-reveal-item=""
            key={panel.id}
          >
            <Link
              href={`/programs/${panel.slug}`}
              className="flex flex-col h-full focus:outline-none focus-visible:ring-2 focus-visible:ring-accent rounded-media"
            >
              {panel.image === null ? null : (
                <figure className="relative aspect-[3/2] overflow-hidden rounded-media xl:aspect-[4/3]">
                  ...
                  />
                  <div className="absolute inset-0 bg-linear-to-t from-surface/80 via-surface/30 to-transparent transition-opacity duration-500 group-hover:opacity-90" />
                  <figcaption className="absolute inset-x-0 bottom-0 flex flex-col gap-1 p-5 lg:p-6 transition-transform duration-500 group-hover:-translate-y-1">
